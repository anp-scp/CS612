...
int main(int argc, char* argv[]){
	...
	i = atoi(argv[1]);
	// Memory Allocation	
	double* A = (double*) malloc((i*i)* sizeof(double));
	double* B = (double*) malloc((i*i)* sizeof(double));
	double* C = (double*) malloc((i*i)* sizeof(double));
	// Assignment of random values
	for(int j=0; j<i*i;j++){
		A[j] = (double)rand()/(double)10000;
		B[j] = (double)rand()/(double)10000;
	}
